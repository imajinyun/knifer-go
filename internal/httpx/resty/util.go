...
func PostString(rawURL, body string) string { return Post(rawURL).BodyString(body).Execute().Body() }
...
func PostJSON(rawURL, jsonStr string) string { return Post(rawURL).BodyJSON(jsonStr).Execute().Body() }
