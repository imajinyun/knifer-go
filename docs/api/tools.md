...
| `CloseCookie` | `func CloseCookie()` | CloseCookie delegates to the internal httpx implementation. | facade | — |
...
| `DownloadStringE` | `func DownloadStringE(rawURL string, customCharset string) (string, error)` | DownloadStringE downloads remote text and returns an error on request or read failure. | facade | — |
...
| `FilterHTMLTag` | `func FilterHTMLTag(s string, tagNames ...string) string` | FilterHTMLTag delegates to the internal httpx implementation. | facade | — |
...
| `HTMLEscape` | `func HTMLEscape(s string) string` | HTMLEscape delegates to the internal httpx implementation. | facade | — |
| `HTMLUnescape` | `func HTMLUnescape(s string) string` | HTMLUnescape delegates to the internal httpx implementation. | facade | — |
...
| `IsRedirected` | `func IsRedirected(status int) bool` | IsRedirected delegates to the internal httpx implementation. | facade | — |
...
| `ParseUserAgent` | `func ParseUserAgent(ua string) *UserAgent` | ParseUserAgent parses a User-Agent string. | facade | — |
...
| `SetCookieJar` | `func SetCookieJar(jar http.CookieJar)` | SetCookieJar delegates to the internal httpx implementation. | facade | — |
...
| `WithResponseReadAllFunc` | `func WithResponseReadAllFunc(readAll func(io.Reader) ([]byte, error)) RequestOption` | WithResponseReadAllFunc sets the reader used by response Bytes/Body helpers. | facade | — |
...
| `WithScopedGlobalConfig` | `func WithScopedGlobalConfig(cfg GlobalConfig, fn func())` | WithScopedGlobalConfig runs fn with cfg installed as package-level HTTP defaults, then restores the previous defaults. | facade | — |
...
| `WithTransport` | `func WithTransport(t http.RoundTripper) RequestOption` | WithTransport sets a per-request RoundTripper and takes precedence over WithTLSConfig. | facade | — |
