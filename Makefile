...
help:
	...
	@echo "  local-governance-gates-check Verify local gate wiring dependencies"
...
local-governance-gates-check:
	$(GO) run ./bin/localgovernancegatescheck -root .

random-source-policy-check:
	$(GO) run ./bin/randomsourcepolicycheck -root .

threat-model-check:
	$(GO) run ./bin/threatmodelcheck -root .
...
api-convergence-check:
	$(GO) run ./bin/apiconvergencecheck -root .

lifecycle-check:
	$(GO) run ./bin/lifecyclecheck -root .
...
docs-gen: tools-gen facade-tiering-gen

docs-check: tools-check docs-quickstart-check

facade-tiering-gen:
	python3 bin/generate_facade_tiering.py
